...
public class ReservaServiceReactivo {
    ...
    /**
     * Buscar reserva por ID de forma reactiva CON recuperación
     */
    public Mono<Reserva> findById(Long id) {
        ...
        return reservaRepository.findById(id)
                .doOnSubscribe(subscription -> log.info("onSubscribe: buscando reserva {}", id))
                .doOnNext(reserva -> log.info("onNext: ✅ Reserva encontrada ID: {}", reserva.getId()))
                .switchIfEmpty(Mono.defer(() -> {
                    log.warn("⚠️ Reserva con ID {} NO EXISTE", id);
                    return Mono.error(new RuntimeException("Reserva no encontrada con ID: " + id));
                }))
                .onErrorResume(error -> {
                    log.warn("================================================");
                    log.warn("🔄 onErrorResume ACTIVADO!");
                    log.warn("   Error: {}", error.getMessage());
                    log.warn("   Acción: Retornando reserva por defecto");
                    log.warn("================================================");

                    Reserva reservaRecuperacion = new Reserva();
                    reservaRecuperacion.setId(-1L);
                    reservaRecuperacion.setHuespedId(-1L);
                    reservaRecuperacion.setHabitacionId(-1L);
                    reservaRecuperacion.setEstado("Error: " + error.getMessage());

                    return Mono.just(reservaRecuperacion);
                });
    }

    /**
     * Buscar reserva por ID SIN recuperación (para demo)
     */
    public Mono<Reserva> findByIdSinRecuperacion(Long id) {
        log.info("BUSCANDO RESERVA (SIN RECUPERACIÓN) ID: {}", id);

        return reservaRepository.findById(id)
                .switchIfEmpty(Mono.defer(() -> {
                    log.error("❌ ERROR: Reserva {} NO EXISTE - El flujo SE DETENDRÁ", id);
                    return Mono.error(new RuntimeException("Reserva no encontrada con ID: " + id));
                }));
    }
                                        ...
                                        .flatMap(pagoGuardado -> {
                                            ...
                                            return habitacionRepository.findById(reservaCompleta.getHabitacionId())
                                                    ...
                                                    .map(habitacionActualizada -> {
                                                        log.info("Habitación actualizada a ocupada: {}",
                                                        ...
                                                        // Datos del huésped
                                                        detallada.setNombreHuesped(huespedGuardado.getNombre() + " "
                                                                + huespedGuardado.getApellido());
                                                        ...
                                                        // Datos de la habitación
                                                        detallada
                                                                .setNumeroHabitacion(habitacionActualizada.getNumero());
                                                        detallada.setTipoHabitacion(habitacionActualizada.getTipo());
                                                        detallada
                                                                .setPrecioHabitacion(habitacionActualizada.getPrecio());
    ...
    public Flux<Reserva> findByHabitacionId(Long habitacionId) {
        ...
        return reservaRepository.findByHabitacionId(habitacionId)
                ...
                .doOnNext(reserva -> log.debug("Reserva encontrada para habitación {}: {}", habitacionId,
                        reserva.getId()))
